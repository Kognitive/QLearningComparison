...
if env == "grid":
    ...
    arg_v_function = np.argmax(q_function, axis=1)
    shaped_arg_v_function = np.reshape(arg_v_function, [N, N])

    arg_v_function = np.argpartition(q_function, 1)
    for x in range(N):
        for y in range(N):
            val = shaped_arg_v_function[x, y]

            hw = 0.3
            hl = 0.15

            if val == 0:
                plt.arrow(x + 0.15, y, -0.15, 0, head_width=0.3, head_length=0.15, fc='w', ec='k', linewidth=0.0, width=0)
            if val == 1:
                plt.arrow(x + 0.15, y, -0.15, 0, head_width=0.3, head_length=0.15, fc='w', ec='k', linewidth=0.0, width=0)
            elif val == 2:
                plt.arrow(x - 0.15, y, 0.15, 0, head_width=0.3, head_length=0.15, fc='w', ec='k', linewidth=0.0, width=0)
...
if env == "expchain":
    ...
    arg_v_function = np.argpartition(q_function, 1)
    shaped_arg_v_function = arg_v_function

    # y = 0
    # for x in range(N):
    #     val = shaped_arg_v_function[x]
    #
    #     hw = 0.3
    #     hl = 0.15
    #
...
if env == "sharedchain":
    ...
    N = 10
    env = SharedLearningChain("grid", [1], N)
    optimal_ih_rew, _, min_q, max_q, q_function = env.get_optimal(200, 0.99)
...
if env == "binflip":
    ...
    for x in range(N):
        for y in range(N):
            val = shaped_arg_v_function[x, y]
            if val == 0:
                plt.arrow(x + 0.2, y, -0.4, 0, head_width=0.15, head_length=0.05, fc='w', ec='w')
            elif val == 1:
                plt.arrow(x, y - 0.2, 0, 0.4, head_width=0.15, head_length=0.05, fc='w', ec='w')
            elif val == 2:
                plt.arrow(x - 0.2, y, 0.4, 0, head_width=0.15, head_length=0.05, fc='w', ec='w')
            elif val == 3:
                plt.arrow(x, y + 0.2, 0, -0.4, head_width=0.15, head_length=0.05, fc='w', ec='w')
